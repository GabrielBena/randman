...
from randman.torch_randman import TorchRandman, RandmanDataset
...
def plot_dataset(dataset, plot3d=False):
    ...
    for i in range(n_classes):
        ...
        if plot3d:
            ret = ax.scatter(
                data[idx, 0], data[idx, 1], data[idx, 2], s=0.5, color=next(colors)
            )
        else:
...
def main():
    dataset = make_classification_dataset(
        2, dim_manifold=2, dim_embedding_space=3, alpha=2.0, nb_samples_per_class=1000
    )
